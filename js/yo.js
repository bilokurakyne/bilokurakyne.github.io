...
function yo(sel) {
    ...
    options.player = (options.title && options.title.indexOf('трейлер')+1 || t)
        ? 'trailer'
        : (!options.player)
            ? 'hdgo,iframe,newvideo,kodik,allserials,moonwalk,trailer,torrent'
            : options.player;
    ...
    options.button = (options.button)
        ? options.button
        : 'hdgo: {Q} {T}, newvideo: {Q} {T}, kodik: {Q} {T}, iframe: {Q} {T}, moonwalk: {Q} {T}';
    if (options.button) {
    ...
    if (parseInt(yohoho.offsetHeight) && parseInt(yohoho.offsetHeight) < 768) {
        if (parseInt(yohoho.parentNode.offsetHeight) && parseInt(yohoho.parentNode.offsetHeight) >= 768) {
